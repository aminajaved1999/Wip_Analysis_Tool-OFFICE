...
namespace WIPAT.BLL
{
    public class WipManager
    {
        ...
        public Response<DataTable> nBuildCommonWipDataTable(List<string> asinList, ForecastMaster forecast1, ForecastMaster forecast2, string targetMonth, string wipType, string capacity = null, int? percentage = null)
        {
            ...
            try
            {
                ...
                foreach (string asin in asinList)
                {
                    ...
                    int actualOrder = context.ActualOrders.Where(a => a.ItemCatalogueId == itemCatalogueId).Select(a => (int?)a.Quantity).FirstOrDefault() ?? 0;

                    int initialStock = context.InitialStocks.Where(s => s.ItemCatalogueId == itemCatalogueId).Select(s => (int?)s.Quantity).FirstOrDefault() ?? 0;
                    ...
                    List<DateTime> poDateList = forecastDict1.Keys.Union(forecastDict2.Keys).ToList();
        ...
        #region save
         ...
         public async Task<Response<bool>> SaveWipRecordsAsync3(DataTable dataTable,string fileName,string wipType,string capacity,string targetMonth,string wipColName)
        {
