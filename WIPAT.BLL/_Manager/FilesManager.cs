...
namespace WIPAT.BLL
{
    public class FilesManager 
    {
        ...
        public FilesManager(IForecastRepository forecastRepository,
                            IStockRepository stockRepository,
                            IOrderRepository orderRepository,
                            IItemsRepository itemsRepository,
                            IMiscellaneousRepository miscellaneousRepository,

                            WipSession session, Action<string> showBusy = null, Action hideBusy = null, Action<string, StatusType> setStatus = null)
        {
            ...
            _miscellaneousRepository = miscellaneousRepository ?? throw new ArgumentNullException(nameof(miscellaneousRepository));
        ...
        public async Task<Response<List<ForecastFileData>>> HandleForecastFileAsync(string filePath, List<ForecastFileData> forecastFiles, int commitmentPeriod, WipSession session)
        {
            ...
            try
            {
                ...
                var checksRes = _forecastRepository.PerformForecastChecks2(forecastFileData.FileName, forecastFileData.ProjectionMonth, forecastFileData.ProjectionYear);
                ...
                else
                {
                    ...
                    var saveResponse = _forecastRepository.SaveForecastDataToDatabase(forecastFileData, isFirstFile);
                    ...
                    var forecastDataRes = _forecastRepository.GetForecastDataFromDB(forecastFileData.ProjectionMonth, forecastFileData.ProjectionYear);
            ...
            try
            {
                using (var package = new ExcelPackage(new FileInfo(filePath)))
                {
                    ...
                    for (int row = 2; row <= ws.Dimension.End.Row; row++)
                    {
                        ...
                        var casinExistResponse = await _itemsRepository.IsCasinExistInCatalogueAndInitialStock(casin);
                    ...
                    var updateStockResponse = await _stockRepository.UpdateStockQuantitiesAsync(existingItemsInCatalogue);
        ...
        public async Task<Response<OrderFileResponse>> HandleOrderFile(string filePath, bool orderUploaded, WipSession session)
        {
            ...
            try
            {
                ...
                if (existingFileRes.Success)
                {
                    ...
                    var orderDataResponse = _orderRepository.GetExistingOrderData(fileName, requiredMonth, requiredYear);
                ...
                // Save New Data
                var orderUpdateResponse = await _orderRepository.SaveOrdersAndUpdateStock(importResponse.Data.ValidOrders);
                ...
                var freshDataResponse = _orderRepository.GetExistingOrderData(fileName, requiredMonth, requiredYear);
            ...
            try
            {
                using (var package = new ExcelPackage(new FileInfo(filePath)))
                {
                    ...
                    while (true)
                    {
                        ...
                        var itemExistResponse = await _itemsRepository.IsItemExistInCatalogue(casin);
                        string rowStatus;
                        ...
                        if (itemExistResponse.Success)
                        {
                            ...
                            var item = _itemsRepository.GetItemByCAsin(casin); // assuming this is not async
            ...
            try
            {
                ...
                using (var package = await Task.Run(() => new ExcelPackage(fileInfo)))
                {
                    ...
                    if (fileType == FileType.Forecast.ToString() || fileType == FileType.Order.ToString() || fileType == FileType.Stock.ToString())
                    {
                        ...
                        {
                            if (!_itemsRepository.IsCAsinExistInCatalogue(c))
                                missing.Add(c);
                        }
        ...
        public async Task<Response<ForecastFileData>> LoadExistingForecastAsync(string month, string year)
        {
            ...
            try
            {
                ...
                var forecastDataRes = await Task.Run(() => _forecastRepository.GetForecastDataFromDB(month, year));
        ...
        public async Task<Response<DataTable>> LoadExistingOrderAsync(string month, string year)
        {
            ...
            try
            {
                // 1. Fetch data from Repository
                // Wrapping in Task.Run ensures the UI doesn't freeze if the repo call is synchronous
                var OrderDataRes = await Task.Run(() => _orderRepository.GetOrderDataByMonthYear(month, year));
