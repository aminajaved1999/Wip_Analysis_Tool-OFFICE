    ...
    public class DataTableFactory
    {
        ...
        public async Task<Response<DataTable>> GetItemCataloguesDataTableFromExcel(string filePath, string requiredWorkSheetName, int loggedInUserId, bool isUpdate = false)
        {
                        ...
                        foreach (var rule in templateRules)
                        {
                            ...
                            try
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        // Dynamically map using the CatalogueItemStatus enum
                                        // TryParse allows for case-insensitive matching against Enum names (Active, Inactive, Invalid)
                                        ...
                                        {
                                            // Fallback: If the value provided in Excel doesn't match the Enum, 
                                            // you can decide to throw an exception or set a default
                                            dr[colName] = 0;
                                        }
                                    }
                                    else
                                    {
                                        // Standard Type Conversion for non-enum columns
            ...
            catch (Exception ex)
            {
                ...
                response.Success = false;
                response.Message =
                    $"Error reading Items Catalogue Data from Excel file: {ex.Message}";
                response.Data = null;
                return response;
